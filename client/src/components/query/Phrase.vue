<template>
  <div :class="'text-definition ' + [highlighted ? 'highlighted' : '']">
    ...
    <template v-else-if="valueType == 'filter' && entity">
      <div v-if="hasKey(entity, 'property')" :class="'property horizontal  relative ' + [index > 0 ? ' ' : '']">
        <Keyword v-if="index > 0" :class="'operator-label '"> {{ operator }}</Keyword>
        <!-- <Static v-else class=" "> if they</Static> -->
      ...
      <template v-else v-for="(child, childIndex) in children(entity)" :key="child.path">
        <div v-if="isOperator(child?.path)" class="operator horizontal ">
          <Keyword class="operator-label">{{ showOperator(path, childIndex) ? operator : "" }}</Keyword>
...
export default defineComponent({
  ...
  methods: {
    showOperator(path: string, index: number): boolean {
