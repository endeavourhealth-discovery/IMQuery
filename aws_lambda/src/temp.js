...
//   const _searchSubStrings = _searchString.split(" ");   //transform searchstring into array of words
...
//   await Promise.all(_searchSubStrings.map(async (subString: any) => {
...
//   _uniqueEntitiesCounted = _uniqueEntities.map((uniqueIri: any) => {
//     return {
//       iri: uniqueIri.iri,
//       rdfsLabel: uniqueIri.rdfsLabel,
//       count: countOccurrences(_allEntities, uniqueIri),
//       naturalLanguageText: ""
//     }
//   });
...
//   // _uniqueEntitiesCounted = _uniqueEntities.map((uniqueIri: any) => {
//   //   uniqueIri.naturalLanguageText = createPhrase(uniqueIri.iri, _uniqueEntitiesCounted) //createPhrase works
//   // });
...
//       return doesArrayContainAllItemsFrom(_allEntitiesUniqueIris, resultTemplate.requiredIris)
//     });
//   } else {
...
//     template.labelEntityIris.forEach((labelIri: any) => {
//       template.label = template.label.replace(`{${labelIri}}`, createPhrase(labelIri, _uniqueEntitiesCounted));
...
//       description: "Create a New Dataset by Extracting Data from Health Records", //description for _modulesData.hits. array [moduleid][task][rdfs:comment]
//       url: `https://im.endeavourhealth.net/#/search?module=${template.moduleId}&?task=${template.taskId}`
//     });
//   });

//   //module [moduleId, rdfs:label], [task -> taskId, rdfs:label, shortLabel], logoId [table_default], title, description,    
//   //
...
//  **/
// function createPhrase(iri: string, metaData: any): string {
//   let _matchingEntities = metaData.filter((item: any) => item.iri === iri);
